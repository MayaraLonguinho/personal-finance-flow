...
- `database/schema.sql`: tipos e restrições da tabela `transacoes`.
...
Para arquivo por caminho, usar `ler_csv(caminho_arquivo)`. Para upload web, usar a rota existente, que salva o arquivo em `uploads/` e lê com `pandas.read_csv`.
