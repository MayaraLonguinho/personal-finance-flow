...
from b_backend.b_src.e_metrics import gerar_metricas_dashboard, buscar_ultimas_transacoes, gerar_insights
from c_generate_rpa.b_transform import processar_upload_csv
from c_generate_rpa.d_load import carregar_transacoes_mysql, limpar_transacoes_mysql, garantir_colunas_usuario
from b_backend.b_src.c_transacoes import buscar_todas_transacoes, criar_transacao, editar_transacao, excluir_transacao
from b_backend.b_src.g_metas import buscar_meta_ativa, criar_meta, atualizar_meta, excluir_meta
from b_backend.b_src.d_categorias import buscar_todas_categorias, criar_categoria, atualizar_categoria, excluir_categoria, obter_estatisticas_categoria, inicializar_categorias_padrao
from b_backend.b_src.i_financial_agent import responder_pergunta as responder_pergunta_local
from b_backend.b_src.j_ai_financial_agent import responder_pergunta_openai
from b_backend.b_src.f_relatorios import obter_relatorio
from b_backend.b_src.h_investimentos import atualizar_investimento, buscar_investimento_por_id, criar_investimento, excluir_investimento, listar_investimentos, obter_resumo_investimentos
from b_backend.b_src.a_auth import criar_usuario, buscar_usuario_por_email, verificar_senha
from b_backend.b_src.b_usuario_contexto import definir_usuario_id, limpar_usuario_id
from b_backend.b_src.k_configuracoes import (
...
app = Flask(
    __name__,
    template_folder=str(PROJECT_ROOT / "a_frontend_webclient" / "b_templates"),
    static_folder=str(PROJECT_ROOT / "a_frontend_webclient" / "a_static"),
...
@login_obrigatorio
def transacoes():
    return render_template('e_transacoes.html')

# Rota de metas - exibe a página de metas
@app.route('/metas')
@login_obrigatorio
def metas():
    return render_template('h_metas.html')

# Rota de categorias - exibe a página de categorias
@app.route('/categorias')
@login_obrigatorio
def categorias():
    return render_template('f_categorias.html')

# Rota de assistente - exibe a página do assistente financeiro
@app.route('/assistente')
@login_obrigatorio
def assistente():
    return render_template('j_assistente.html')

@app.route("/relatorios")
@login_obrigatorio
def pagina_relatorios():
    return render_template("g_relatorios.html")


@app.route("/investimentos")
@login_obrigatorio
def pagina_investimentos():
    return render_template("i_investimentos.html")


@app.route("/configuracoes")
@login_obrigatorio
def pagina_configuracoes():
    return render_template("k_configuracoes.html")
...
@app.route("/cadastro")
def pagina_cadastro():
    return render_template("b_cadastro.html")
