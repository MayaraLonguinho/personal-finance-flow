...
def buscar_categoria_por_id(id_categoria):
    ...
    try:
        ...
        query = text("SELECT * FROM categorias WHERE id = :id")
        
        with engine.connect() as conn:
            result = conn.execute(query, {'id': id_categoria})
            row = result.fetchone()
...
def excluir_categoria(id_categoria):
    """
    Exclui uma categoria da tabela categorias pelo ID.
    Move transações dessa categoria para 'outros' na mesma transação.
    ...
    Returns:
        dict: Dicionário com sucesso, mensagem e quantidade de transações atualizadas
    """
    try:
        ...
        if categoria['nome'].lower() == 'outros':
            return {'sucesso': False, 'erro': 'Não é permitido excluir a categoria "outros"'}
        
        engine = obter_engine()
        
        # Na mesma transação: atualiza transações e exclui categoria
        with engine.begin() as conn:
            ...
            result_update = conn.execute(query_update, {'nome': categoria['nome']})
            quantidade_atualizada = result_update.rowcount
            
            # Depois, exclui a categoria
            query_delete = text("DELETE FROM categorias WHERE id = :id")
            
            result_delete = conn.execute(query_delete, {'id': id_categoria})
        ...
        return {
            ...
            'quantidade_transacoes_atualizadas': quantidade_atualizada
        }
