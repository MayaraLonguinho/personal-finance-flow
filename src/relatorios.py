...
def validar_data(data_texto: Optional[str]) -> Optional[str]:
    if not data_texto:
        return None
...
def obter_relatorio(
    ...
    if data_inicio:
        filtros.append("DATE(data_transacao) >= :data_inicio")
        parametros["data_inicio"] = data_inicio

    if data_fim:
        filtros.append("DATE(data_transacao) <= :data_fim")
    ...
    consulta_resumo = text(
        ...
        SELECT
            ...
            COALESCE(
                SUM(
                    CASE
                        WHEN tipo = 'investimento' THEN valor
                        ELSE 0
                    END
                ),
                0
    ...
    consulta_categorias = text(
        ...
        FROM transacoes
        {where_sql}
        {"AND" if where_sql else "WHERE"} tipo = 'saida'
        GROUP BY categoria
        ORDER BY valor DESC
        """
    )
    ...
    total_entradas = float(
        resultado_resumo["total_entradas"] or 0
    )

    total_saidas = float(
        resultado_resumo["total_saidas"] or 0
    )

    total_investido = float(
        resultado_resumo["total_investido"] or 0
    )

    quantidade_transacoes = int(
        resultado_resumo["quantidade_transacoes"] or 0
    )
    ...
    for categoria in resultado_categorias:
        categorias.append(
            {
                "categoria": categoria["categoria"] or "Outros",
                "valor": float(categoria["valor"] or 0),
                "quantidade": int(categoria["quantidade"] or 0),
            }
        )
