...
let ultimosDadosGraficos = null;

/**
 * Monitora mudanças de tema/acessibilidade
 * Recarrega os gráficos quando o tema muda
 */
function setupThemeChangeListener() {
    window.addEventListener('pff:theme-changed', function(event) {
        // Aguarda um pouco para CSS ser aplicado
        setTimeout(() => {
            // Se temos dados dos gráficos salvos, recria-os
            if (ultimosDadosGraficos) {
                // Limpa gráficos existentes
                if (window.PFF.accessibility && window.PFF.accessibility.clearChartInstances) {
                    window.PFF.accessibility.clearChartInstances();
                }

                // Recarrega a dashboard
                buscarMetricas().catch(erro => {
                    console.error('Erro ao recarregar gráficos:', erro);
                });
            }
        }, 100);
    });
...
function criarGraficoCategorias(dados) {
    ...
    const chartInstance = new Chart(contexto, {
        type: "doughnut",
    ...
    // Registra instância para atualização de tema
    if (window.PFF && window.PFF.accessibility) {
        window.PFF.accessibility.registerChartInstance(chartInstance);
    }
}
