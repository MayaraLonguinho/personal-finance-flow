...
Todas as tabelas usam `usuario_id`, mas `h_database/a_schema.sql` não possui nenhuma `FOREIGN KEY`. No schema atual, `usuario_id` é NOT NULL em todas as tabelas financeiras.
...
O ciclo de vida, a normalização do nome e a política de retenção dos uploads precisam ser decisões explícitas. O projeto atual apenas mantém `g_uploads/.gitkeep` no versionamento.
