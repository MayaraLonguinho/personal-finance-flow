...
|---|---|
| `b_backend/a_app.py` | Aplicação Flask, páginas, APIs, sessão e orquestração |
| `b_backend/b_src/` | Regras de domínio, acesso ao banco, ETL, métricas e agentes |
| `a_frontend_webclient/b_templates/` | Páginas Jinja |
| `a_frontend_webclient/a_static/` | CSS, JavaScript e imagens |
| `h_database/a_schema.sql` | Definição declarativa das tabelas MySQL |
| `data/raw/` | CSV de entrada de exemplo |
| `data/processed/` | CSV tratado de exemplo |
| `g_uploads/` | Destino dos CSVs enviados pela aplicação |
...
`README.md`, `requirements.txt`, `b_backend/a_app.py`, `h_database/a_schema.sql`, módulos de `b_backend/b_src/`, templates e scripts de `a_frontend_webclient/a_static/b_js/`.
