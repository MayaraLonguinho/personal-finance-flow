...
O schema usa MySQL e define seis tabelas. As relações abaixo são lógicas, inferidas pelo uso de `usuario_id`; o arquivo `h_database/a_schema.sql` não declara chaves estrangeiras.
...
`h_database/a_schema.sql`, `b_backend/b_src/a_auth.py`, `b_backend/b_src/c_transacoes.py`, `b_backend/b_src/g_metas.py`, `b_backend/b_src/d_categorias.py`, `b_backend/b_src/h_investimentos.py`, `b_backend/b_src/k_configuracoes.py` e `b_backend/b_src/e_metrics.py`.
