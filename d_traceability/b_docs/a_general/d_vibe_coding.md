...
- arquivos de `d_traceability/a_brain/`, `d_traceability/b_docs/`, `d_traceability/d_skills/`, `d_traceability/c_mcp/` e `e_verify/`;
...
Também foi preparado um servidor MCP local para uso no Windsurf. A entrada versionada em `d_traceability/c_mcp/e_configuracao_exemplo.json` inicia `d_traceability/c_mcp/a_server.py` por `stdio` e referencia um arquivo local de ambiente. A instalação e a ativação são descritas em `d_traceability/c_mcp/f_readme.md`.
...
3. revisar o projeto e preencher o vault técnico em `d_traceability/a_brain/`;
...
O assistente financeiro existente na aplicação é outro uso de IA, distinto do agente de desenvolvimento. `b_backend/b_src/j_ai_financial_agent.py` usa a OpenAI com function calling; `b_backend/b_src/i_financial_agent.py` oferece fallback local determinístico, sem LLM.
